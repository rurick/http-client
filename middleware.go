...
func (mc *MiddlewareChain) GetMiddlewares() []Middleware {
	return slices.Clone(mc.middlewares)
}
...
func (lm *LoggingMiddleware) Process(req *http.Request, next func(*http.Request) (*http.Response, error)) (*http.Response, error) {
	...
	// Автоматическая запись middleware метрик
	if collector, ok := req.Context().Value("otel_collector").(*OTelMetricsCollector); ok && collector != nil {
		collector.RecordMiddlewareDuration(req.Context(), "logging", duration)
		if err != nil {
			collector.RecordMiddlewareError(req.Context(), "logging", "request_failed")
		}
	}
